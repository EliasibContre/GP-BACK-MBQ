...
export async function createAccessRequest(data) {
  ...
  if (pending) {
    const err = new Error("Ya existe una solicitud pendiente para este correo");
    err.status = 409;
    throw err;
  }
  ...
  if (tipo === "usuario") {
    ...
    if (subtipo === "fisica") {
      ...
      const err = new Error("Subtipo de proveedor inválido");
      err.status = 400;
      throw err;
    }
  ...
  if (kind === "INTERNAL") {
    ...
    if (!department) {
      const err = new Error("Área requerida");
      err.status = 400;
      throw err;
    }
    ...
    if (personType === "MORAL" && !companyName) {
      const err = new Error("Empresa requerida");
      err.status = 400;
      throw err;
    }
    if (!RFC) {
      const err = new Error("RFC requerido");
      err.status = 400;
      throw err;
    }
  }
  ...
  try {
    ...
    const requesterName =
      fullName ||
      companyName ||
      (kind === "INTERNAL" ? "Usuario" : "Proveedor");
    ...
    for (const user of adminUsers) {
      await createNotification({
        ...
        message: `${requesterName} (${email}) envió una solicitud de acceso.`,
        data: {
    ...
    console.error(
      "Error creando notificaciones de solicitud de acceso:",
  ...
  if (String(process.env.MAILER_DISABLED || "false") !== "true") {
    try {
      await sendAccessRequestAckEmail(email, kind);
    } catch (_) {
      // ignore
    }
  } else {
...
export async function getAccessRequest(id) {
  return prisma.accessRequest.findUnique({
    ...
    include: {
      ...
      createdUser: { select: { id: true, email: true, fullName: true } },
    },
  });
}
...
export async function decideAccessRequest({
  ...
  if (decision === "REJECTED") {
    if (!notes || !notes.trim()) {
      const err = new Error("Motivo requerido para rechazo");
      err.status = 400;
      throw err;
    }
    ...
    if (String(process.env.MAILER_DISABLED || "false") !== "true") {
      try {
        await sendAccessRequestRejectedEmail(request.email, notes);
      } catch (e) {
        console.error("Error email rechazo:", e.message);
      }
    } else {
    }
  ...
  if (request.kind === "INTERNAL") {
    if (!department) {
      const err = new Error("Departamento requerido");
      err.status = 400;
      throw err;
    }
    ...
    if (!roles || roles.length === 0) {
      const err = new Error("Asignar al menos un rol");
      err.status = 400;
      throw err;
    }
    userRoles = roles;
  ...
  if (roleRecords.length !== userRoles.length) {
    const err = new Error(
      `Rol inválido: esperados ${userRoles.join(", ")}, encontrados ${roleRecords
        .map((r) => r.name)
        .join(", ")}`,
    );
    err.status = 400;
  ...
  if (String(process.env.MAILER_DISABLED || "false") !== "true") {
    try {
      await sendTemporaryPasswordEmail(request.email, tempPassword);
    } catch (e) {
      console.error("Error email temp password:", e.message);
    }
  } else {
  }
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entity: "AccessRequest",
      entityId: request.id,
      meta: { roles: userRoles, department: userDepartment },
    },
  });
  ...
  return {
    ...
    tempPassword:
      process.env.MAILER_DISABLED === "true" ? tempPassword : undefined,
  };
}
