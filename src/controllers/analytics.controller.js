...
export async function getDashboardStats(req, res) {
  try {
    const [proveedoresAprobados, proveedoresRechazados] = await Promise.all([
    ...
    const facturasAprobadas =
      purchaseOrdersWithInvoices.find((po) => po.status === "APPROVED")
        ?._count || 0;

    const facturasRechazadas =
      purchaseOrdersWithInvoices.find((po) => po.status === "CANCELLED")
        ?._count || 0;
    ...
    const ordenesAprobadas =
      purchaseOrdersByStatus.find((po) => po.status === "APPROVED")?._count ||
      0;

    const ordenesRechazadas =
      purchaseOrdersByStatus.find((po) => po.status === "CANCELLED")?._count ||
      0;

    const ordenesRetrasadas =
      purchaseOrdersByStatus.find((po) => po.status === "SENT")?._count || 0;
    ...
    const documentosNuevos =
      documentsByStatus.find((d) => d.status === "PENDING")?._count || 0;

    const documentosEnAviso =
      documentsByStatus.find((d) => d.status === "EXPIRED")?._count || 0;

    const documentosVencidos =
      documentsByStatus.find((d) => d.status === "REJECTED")?._count || 0;
  ...
  try {
    ...
    orders.forEach((order) => {
      ...
      if (order.status === "DRAFT") {
        daysInStatus = order.issuedAt
          ? getDaysDifference(order.createdAt, order.issuedAt)
          : 0;
      } else if (order.status === "SENT") {
        daysInStatus = order.approvedAt
          ? getDaysDifference(order.issuedAt, order.approvedAt)
          : getDaysDifference(order.issuedAt, new Date());
      } else if (order.status === "APPROVED") {
        daysInStatus = order.receivedAt
          ? getDaysDifference(order.approvedAt, order.receivedAt)
...
export async function getProviderDashboardStats(req, res) {
  try {
    ...
    const facturasCerradas = todasLasOrdenes.filter(
      (o) => o.status === "APPROVED"
    ).length;
    const facturasRetrasadas = todasLasOrdenes.filter(
      (o) => o.status === "CANCELLED"
    ).length;
    const facturasVolumenActivo = todasLasOrdenes.filter(
    ...
    const ordenesCerradas = todasLasOrdenesCompra.filter(
      (o) => o.status === "APPROVED"
    ).length;
    const ordenesRetrasadas = todasLasOrdenesCompra.filter(
      (o) => o.status === "CANCELLED"
    ).length;
    const ordenesVolumenActivo = todasLasOrdenesCompra.filter(
      (o) => o.status === "SENT" || o.status === "DRAFT"
    ).length;
...
export async function getApproverDashboardStats(req, res) {
  try {
    ...
    const isApprover =
      roles.includes("APPROVER") || roles.includes("APROBADOR");

    if (!isApprover) {
      return res.status(403).json({
        error: "No autorizado como aprobador",
      });
    }
    ...
    console.error("Error obteniendo estadísticas del aprobador:", error);
    return res.status(500).json({
      error: "Error al obtener estadísticas del aprobador",
      message: error.message,
    });
  }
}
